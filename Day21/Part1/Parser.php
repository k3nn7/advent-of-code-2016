...
class Parser
{
    public function parse(string $input): array
    {
        ...
        if (preg_match('/^swap letter (?P<X>\d+) with letter (?P<Y>\d+)$/', $input, $matches)) {
            return [Operation::SWAP_LETTER, (int)$matches['X'], (int)$matches['Y']];
        }
        if (preg_match('/^rotate left (?P<X>\d+) steps$/', $input, $matches)) {
            return [Operation::ROTATE_LEFT, (int)$matches['X']];
        }
        if (preg_match('/^rotate right (?P<X>\d+) steps$/', $input, $matches)) {
            return [Operation::ROTATE_RIGHT, (int)$matches['X']];
        }
        if (preg_match('/^rotate based on position of letter (?P<X>\d+)$/', $input, $matches)) {
            return [Operation::ROTATE_BASED, (int)$matches['X']];
        }
